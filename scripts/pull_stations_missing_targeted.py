...
def date_bounds_for_file(year: int, file_name: str, df: pd.DataFrame) -> tuple[pd.Timestamp, pd.Timestamp]:
    start = pd.Timestamp(f"{year}-01-01T00:00:00", tz="Asia/Kolkata")
    end = pd.Timestamp(f"{year}-12-31T23:59:59", tz="Asia/Kolkata")
    if file_name == "2026_partial.parquet":
        if len(df):
            end = pd.to_datetime(df.index.max()).tz_convert("Asia/Kolkata")
        else:
            end = pd.Timestamp("2026-03-31T23:59:59", tz="Asia/Kolkata")
    return start, end
